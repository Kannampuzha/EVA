...
class Organism():
    ...
    def PF(self):
        ...
        if recurse_or_terminate == 0:
            ...
            # That means, if 0, then further possible function will work on the same stimuli as before.
            # If 1, then future function will work on a new stimuli, which was the response from this current function.
    ...
    def non_PF(self):
        ...
        # making its set, where everyone is unique
        ...
        # selecing one with highest score
        difrence_threshold = 5  # change this to increase diffrnece.
        # It is an added bias to the alogoritm, roughly translating to "the good one should be atleast 3 scores ahead of all the other ones".
        ...
        # checking if the gap between chosen function and others is equal or greater than the diffence threshold.
        for x in past_instances_with_score:
            if x[0] != self.selected_instance and self.selected_score != 0:
                if self.selected_score - x[1] == 0:
                    ...
                    continue
                    #This means 2 or more sessions are equal. We can choose one randomly from them.
        ...
        for x in selected_past_instace:
            ...
            #So we map this in operations_to_be_performed, to direct in afterwards to work on self.response and not self.stimuli.
        ...
        # performing the operations as per the sequnece. Wherever its 1, the stimuli is changed to the eariler response.
        # Wherever it is 0, stimuli is turned back to the initial stimuli.
